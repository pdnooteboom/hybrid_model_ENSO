...
def J48(train_set, test_set , CV_folds = 10, C = 0.25, M = 5 ,out_file = "results" , model_file = "tmp.model", model_dir = '', print_feat = 0):
    ...
    """
    wekapath = os.getcwd()[:-8]
    
    command = "java -classpath "+ wekapath +"weka-3-7-12/weka.jar weka.classifiers.trees.J48  -C " + str(C) + " -M " + str(M) + " -x " + str(CV_folds) + " -t " + train_set + ".arff "
    command += " -d " + model_dir + "tmp.model"
    args = shlex.split(command)
    p1 = subprocess.Popen(args, stdout=subprocess.PIPE)
    output = p1.communicate()[0]
    command2 = "java -classpath "+ wekapath +"weka.jar weka.classifiers.trees.J48  -l " + model_dir + "tmp.model -T " + test_set + '.arff -p ' + str(print_feat)
...
def NN_regression(train_set, test_set , out_file = "results" , model_file = "tmp.model" ,model_dir = '', learn_rate = 0.3 , momentum = 0.2, train_time = 500 , layers = "a", print_feat = 1):
    ...
    command = "java -classpath "+ wekapath +"weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -L " + str(learn_rate) + " -M " + str(momentum)
    command += " -N " + str(train_time) + " -V 0 -S 0 -E 20 -H " + _layers(layers) + " -t " + train_set + ".arff "
    command += " -d " + model_dir + model_file + ".model"
    args = shlex.split(command)
    p1 = subprocess.Popen(args, stdout=subprocess.PIPE)
    output = p1.communicate()[0]
    command2 = "java -classpath "+ wekapath +"weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -l " + model_dir + model_file + ".model -T " + test_set + '.arff -p '  + str(print_feat)
    args2 = shlex.split(command2)
    p2 = subprocess.Popen(args2, stdout=subprocess.PIPE)
    output2 = p2.communicate()[0]
    fil = open(model_dir + out_file + '.arff', "w")
