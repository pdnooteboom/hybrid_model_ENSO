...
def forecasts(ts,steps,params,arparams,resid, ARIMA_ts):
    """Forecast from the end of ts, steps steps in the future, according to the 
    coefficients params"""
    res = np.zeros(steps)
    ns = ts
    ...
    for i in range(steps):
        for j in range(len(params)):
            if(j==0):
                res[i] += params[0]
            elif(j>0 and j<=len(arparams)):
                res[i] += params[j]*ns[-(j-1)]
            else:
                if(len(ARIMA_ts)>i):
                    res[i] += params[j]*(ARIMA_ts[-1-i]-res[i-1])
        ns = np.append(ns,res[i])
    return res[-1]
...
def NN_regression(train_set, test_set , out_file = "results" , model_file = "tmp.model" ,model_dir = '', learn_rate = 0.3 , momentum = 0.2, train_time = 500 , layers = "a", print_feat = 1):
    ...
    :return: returns the parsed  classified result
    """
    command = "java -classpath C:/Users/User/Documents/Thesis/Hybrid_prediction_model/ClimateLearn/weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -L " + str(learn_rate) + " -M " + str(momentum)
    command += " -N " + str(train_time) + " -V 0 -S 0 -E 20 -H " + _layers(layers) + " -t " + train_set + ".arff "
    command += " -d " + model_dir + model_file + ".model"
    args = shlex.split(command)
    p1 = subprocess.Popen(args, stdout=subprocess.PIPE)
    output = p1.communicate()[0]
    command2 = "java -classpath C:/Users/User/Documents/Thesis/Hybrid_prediction_model/ClimateLearn/weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -l " + model_dir + model_file + ".model -T " + test_set + '.arff -p '  + str(print_feat)
    args2 = shlex.split(command2)
    p2 = subprocess.Popen(args2, stdout=subprocess.PIPE)
    output2 = p2.communicate()[0]
    fil = open(model_dir + out_file + '.arff', "w")
...
def NN_classification(train_set, test_set , out_file = "results" , model_file = "tmp_model", model_dir = '' , learn_rate = 0.3 , momentum = 0.2, train_time = 500, layers = "a", print_feat = 1):
    ...
    """
    command = "java -classpath C:/Users/User/Documents/Thesis/Code_in_thesis/ClimateLearn/weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -L " + str(learn_rate) + " -M " + str(momentum) 
    command += " -N " + str(train_time) + " -V 0 -S 0 -E 20 -H " + layers + " -t " + train_set + ".arff "
    command += " -d " + model_dir + model_file +".model"
    args = shlex.split(command)
    p1 = subprocess.Popen(args, stdout=subprocess.PIPE)
    output = p1.communicate()[0]
    command2 = "java -classpath C:/Users/User/Documents/Thesis/Code_in_thesis/ClimateLearn/weka-3-7-12/weka.jar weka.classifiers.functions.MultilayerPerceptron  -l " + model_dir + model_file + ".model -T " + test_set + '.arff -p '  + str(print_feat)
    args2 = shlex.split(command2)
    p2 = subprocess.Popen(args2, stdout=subprocess.PIPE)
    output2 = p2.communicate()[0]
    fil = open(model_dir + out_file + '.arff', "w")
